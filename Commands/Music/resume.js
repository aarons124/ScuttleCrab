...
module.exports = class ResumeCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      commandAliases: [],
      commandDescription:
        'Quita el pausado de la canción actual en el servidor',
      commandUsage: 'resume',
      commandUsageExample: 'resume',
  ...
  async run(message, args) {
    ...
    try {
      ...
      if (!player.pause) {
        const embed_nopaused = new MessageEmbed()
          .setColor(client._colors.red)
          .setDescription(
            `${client._emojis.no_entry_sign} La canción no está pausada.`
          );
        return message.channel.send({ embeds: [embed_nopaused] });
      }
      ...
      message.channel.send({ embeds: [embed_resume] });
    } catch (error) {
      console.error(
